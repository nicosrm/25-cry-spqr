...
\begin{frame}{PQXDH $\blacktriangleright$ Einführung}
    \begin{itemize}
        ...
        \note<2>{
            PQ-Sicherheit
            \begin{itemize}
                \item Output muss für O\&E zufällig aussehen $\to$ zufällig aussehende Keys $\Rightarrow$ HNDL-Schutz
                \item zukünftig: Schutz der Ratchet (Teaser: SQPR)
            \end{itemize}
        }
        
        \item Kombination von bisherigem Ecliptic Curve (EC) und CRYSTALS-Kyber
        \note<2>{
            Erweiterung von X3DH
            \begin{itemize}
...
\begin{frame}{PQXDH $\blacktriangleright$ Rollen im Key Agreement \footnotesize\cite{kretPQXDHKeyAgreement2024}}
    ...
    \note{
        \begin{itemize}
            \item Alice
            \begin{itemize}
                \item Senden von initialen, verschlüsselten Daten an Bob
                \item Etablierung eines Shared Key → Verwendung für bidirektionale Kommunikation
            \end{itemize}
            
            \item Bob
            \begin{itemize}
                \item möchte es Parteien wie Alice erlauben, dass Shared Key etabliert und verschlüsselte Daten gesendet werden
                \item ggf. offline, wenn Alice dies versucht → Beziehung zu einem Server
            \end{itemize}
            
            \item Server
            \begin{itemize}
                \item Speicherung von Nachrichten von Alice an Bob; späterer Abruf durch Bob
...
\begin{frame}{PQXDH $\blacktriangleright$ Design-Idee \footnotesize\cite{bhargavanAnalysisSignalsPQXDH2023, kretQuantumResistanceSignal2023,kretPQXDHKeyAgreement2024}}
    \begin{itemize}
        ...
        \item Verwendung von $SS_{\text{KEM}}$ und Diffie-Hellman-Werten $DH_{\{1,\ldots,4\}}$ aus X3DH in\\\emph{Key Derivation Function} (KDF)
...
\begin{frame}{PQXDH $\blacktriangleright$ I. Veröffentlichung der Keys \footnotesize\cite{kretPQXDHKeyAgreement2024}}
    ...
    \note{
        \begin{itemize}
            ...
            \item $PQSPK_B$ als \textbf{Last Resort} -- nur, wenn keine $PQOPK$ mehr verfügbar sind
        \end{itemize}
    }
\end{frame}
...
\begin{frame}{PQXDH $\blacktriangleright$ II. Initiale Nachricht senden \footnotesize\cite{bhargavanAnalysisSignalsPQXDH2023,kretPQXDHKeyAgreement2024,schmidtAnalysisSignalsPQXDH2024}}
    ...
    \note{
        \begin{itemize}
            \item Gesamt-Prozess für Phase II.
...
\begin{frame}{PQXDH $\blacktriangleright$ II. Initiale Nachricht senden \footnotesize\cite{bhargavanAnalysisSignalsPQXDH2023,kretPQXDHKeyAgreement2024,schmidtAnalysisSignalsPQXDH2024}}
    ...
    \note{
        \begin{itemize}
            ...
            \item PQKEM One-Time Signed Prekey $PQOTP_B$ falls vorhanden
            \begin{itemize}
            ...
            \end{itemize}
            \item Server löscht OTPs anschließend $\to$ \emph{signle use}
...
\begin{frame}{PQXDH $\blacktriangleright$ II. Initiale Nachricht senden \footnotesize\cite{bhargavanAnalysisSignalsPQXDH2023,kretPQXDHKeyAgreement2024,schmidtAnalysisSignalsPQXDH2024}}
    ...
    \note{
        \begin{itemize}
            \item Generierung eines Ephemeral EC Schlüsselpaares mit Public Key $EK_A$, Berechnung DH-Werte (vgl. X3DH)
...
\begin{frame}{PQXDH $\blacktriangleright$ II. Initiale Nachricht senden \footnotesize\cite{bhargavanAnalysisSignalsPQXDH2023,kretPQXDHKeyAgreement2024,schmidtAnalysisSignalsPQXDH2024}}
    ...
    \note<2>{
        \begin{itemize}
            \item berechnete Werte $DH_{\{1,\ldots,4\}}$ und $SK_{\text{KEM}}$ in KDF $\Rightarrow$ Output: \emph{Shared Key} $SK$
            \begin{itemize}
                \item falls kein $OTP_B$ und damit kein $DH_4$, dann hier weglassen
            \end{itemize}
        \end{itemize}
    }
        ...
        \begin{itemize}
            ...
            \begin{itemize}
                ...
                \begin{itemize}
                    ...
                    \item Warum? $\to$ Verhinderung einer \emph{KEM Re-Enapsulation} Attacke (vgl. \cite{bhargavanAnalysisSignalsPQXDH2023,schmidtAnalysisSignalsPQXDH2024})
                \end{itemize}

                \item optional: weiter Infos dranhängen
            \end{itemize}

            \item Verschlüsselung eines neuen Ciphertexts $CT_{init}$ unter Verwendung von $SK$ und $AD$
        \end{itemize}
    }
    ...
    \note<4>{
        \begin{itemize}
            ...
            \item Alice: Löschen von $CT_{\text{KEM}}$
...
\begin{frame}{PQXDH $\blacktriangleright$ III. Empfangen der initialen Nachricht \footnotesize\cite{bhargavanAnalysisSignalsPQXDH2023,kretPQXDHKeyAgreement2024,schmidtAnalysisSignalsPQXDH2024}}
    ...
    \note<2>{
        \begin{itemize}
            \item Wdh. der KDF- und AD-Berechnungen $\Rightarrow$ Berechnung von Shared Key $SK$ sowie Associated Data $AD$
            ...
            \item Forward Secrecy: löschen vom Ciphertext und allen verwendeten OTPs
        \end{itemize}
    }
\end{frame}
...
\begin{frame}{PQXDH $\blacktriangleright$ Sicherheitsanalyse \footnotesize\cite{kretPQXDHKeyAgreement2024, schmidtAnalysisSignalsPQXDH2024}}
    ...
    \note<1>{
        ...
        \begin{itemize}
            ...
            \item Fix: KEM muss Public Key des Empfängers beinhalten, sonst Hinzufügen bei $AD$
        \end{itemize}
    }
