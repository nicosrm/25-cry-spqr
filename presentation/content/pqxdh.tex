...
\begin{frame}{PQXDH $\blacktriangleright$ Einführung}
    \begin{itemize}
        ...
        \item Ziel: Verhinderung von \emph{Harvest Now, Decrypt Later} Attacken
...
\begin{frame}{PQXDH $\blacktriangleright$ Design-Idee \footnotesize\cite{bhargavanAnalysisSignalsPQXDH2023, kretQuantumResistanceSignal2023,kretPQXDHKeyAgreement2024}}
    ...
    \begin{figure}
        ...
        \caption{Ermittlung vom Shared Key $SK$}
    \end{figure}
    \vspace{-2em}
\end{frame}
...
\begin{frame}{PQXDH $\blacktriangleright$ II. Initiale Nachricht senden \footnotesize\cite{bhargavanAnalysisSignalsPQXDH2023,kretPQXDHKeyAgreement2024,schmidtAnalysisSignalsPQXDH2024}}
    \begin{figure}
        \includegraphics[height=5.7cm]{assets/pqxdh_complete_alice.drawio.pdf}
        \caption{Ablauf von Phase II. und III. von PQXDH (orientiert an \cite{bhargavanAnalysisSignalsPQXDH2023})}
    \end{figure}
...
\begin{frame}{PQXDH $\blacktriangleright$ II. Initiale Nachricht senden \footnotesize\cite{bhargavanAnalysisSignalsPQXDH2023,kretPQXDHKeyAgreement2024,schmidtAnalysisSignalsPQXDH2024}}
    ...
    \note{
        \begin{itemize}
            ...
            \item {\bf Farben erklären!}
            \item PQKEM One-Time Signed Prekey $PQOPK_B$ falls vorhanden
            \begin{itemize}
                \item falls nicht, dann \emph{Last Resort Signed Prekey} $PQSPK_B$
...
\begin{frame}{PQXDH $\blacktriangleright$ II. Initiale Nachricht senden \footnotesize\cite{bhargavanAnalysisSignalsPQXDH2023,kretPQXDHKeyAgreement2024,schmidtAnalysisSignalsPQXDH2024}}
    \begin{figure}
        \includegraphics[height=5.7cm]{assets/pqxdh_alice_dh_pqkem.drawio.pdf}
        \caption{Ermittlung X3DH- und PQKEM-Output von Alice (orientiert an \cite{schmidtAnalysisSignalsPQXDH2024})}
    \end{figure}
...
\begin{frame}{PQXDH $\blacktriangleright$ II. Initiale Nachricht senden \footnotesize\cite{bhargavanAnalysisSignalsPQXDH2023,kretPQXDHKeyAgreement2024,schmidtAnalysisSignalsPQXDH2024}}
    \only<beamer>{
        ...
        \framezoom<0><2>(1.7cm,2.35cm)(8cm,3cm)
...
\begin{frame}{PQXDH $\blacktriangleright$ III. Empfangen der initialen Nachricht \footnotesize\cite{bhargavanAnalysisSignalsPQXDH2023,kretPQXDHKeyAgreement2024,schmidtAnalysisSignalsPQXDH2024}}
    ...
    $\Rightarrow$ Rückwärtsrechnen von Alice Schritten
\end{frame}


\begin{frame}{PQXDH $\blacktriangleright$ III. Empfangen der initialen Nachricht \footnotesize\cite{bhargavanAnalysisSignalsPQXDH2023,kretPQXDHKeyAgreement2024,schmidtAnalysisSignalsPQXDH2024}}
    \begin{figure}
        \includegraphics[height=5.7cm]{assets/pqxdh_bob_dh_pqkem.drawio.pdf}
        \caption{Ermittlung X3DH- und PQKEM-Output von Bob (orientiert an \cite{schmidtAnalysisSignalsPQXDH2024})}
    \end{figure}
    ...
    \note{
        \begin{itemize}
            ...
            \item Decapsulate statt Encapsulate $\to$ auspacken statt verpacken
...
\begin{frame}{PQXDH $\blacktriangleright$ III. Empfangen der initialen Nachricht \footnotesize\cite{bhargavanAnalysisSignalsPQXDH2023,kretPQXDHKeyAgreement2024,schmidtAnalysisSignalsPQXDH2024}}
    % \framezoom(upper left x,upper left y)(zoom area width,zoom area depth)
    \framezoom<0><1>[border](3.6cm,1.76cm)(8.5cm,3.8cm)
...
\begin{frame}{PQXDH $\blacktriangleright$ Sicherheitsanalyse \footnotesize\cite{bhargavanAnalysisSignalsPQXDH2023,kretPQXDHKeyAgreement2024, schmidtAnalysisSignalsPQXDH2024}}
    \begin{itemize}
        \item Formale Verifikation $\to$ Finden von Angriffsmöglichkeiten
        
        \item Bspw. \emph{Key Re-Encapsulation} $\to$ \emph{Man-in-the-Middle Attack}
        \begin{itemize}
            \item[$\Rightarrow$] $AD = \text{EncodeEC}(IK_A) \circ \text{EncodeEC}(IK_B) \circ \alert{\text{EncodeKEM}(PQPK_B)}$
        \end{itemize} 
    \end{itemize}
    ...
    \note<1>{
        \begin{itemize}
            ...
            \item Hinzufügen von PQ-Schutz mehr als nur Einbringen von PQ-Krypto, viele \emph{Pitfalls}
            
            \item Key-Re-Encapsulation: Kompromittierung von $PQPK_B \to$ \emph{Man-in-the-Middle-Attack}
            \begin{itemize}
                \item O\&E als Man-in-the-Middle: Austausch von $PQPK$ durch kompromittierten Schlüssel
                \item bei Kommunikation: Entpacken, Lesen, ggf. Austausch der Nachricht, Senden
                \item Fix: KEM muss Public Key des Empfängers beinhalten, sonst Hinzufügen bei $SS$-Bestimmung hinzufügen, bpsw. bei $AD$
            \end{itemize}

            \item Key Compromise $\to$ Impersonation $\to$ Ratchet
        \end{itemize}
    }
