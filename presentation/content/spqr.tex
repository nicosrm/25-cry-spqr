...
\begin{frame}{SPQR $\blacktriangleright$ Forward Secrecy und Post-Compromise Security \cite{connellSignalProtocolPostQuantum2025}}
    \begin{itemize}
        ...
        \item FS und PCS mittels \emph{Continues Key Agreement} (CKA)
        \begin{itemize}
    ...
    \textbf{ML-KEM}
    \begin{itemize}
        \item A, with every message she sends, sends an EK
        \item B, with every message he receives, generates a secret and a CT, and sends the CT back
        \item A, on receiving a CT, extracts the secret with her DK and mixes it in
    \end{itemize}
\end{frame}

\begin{frame}{SPQR $\blacktriangleright$ Problematik \cite{connellSignalProtocolPostQuantum2025}}
    \begin{columns}
        \begin{column}{0.5\textwidth}
            ...
            \begin{itemize}
                \item ML-KEM basiert auf geordnetem asymmetrischem Austausch
                    \begin{itemize}
                        \item \emph{State Machine}
                    \end{itemize}
            \end{itemize}
            }<1-4>
            \only{
            \begin{itemize}
                \item EK und CT sehr groß
                \begin{itemize}
        ...
        \begin{column}{0.5\textwidth}
            \only{
            \begin{figure}
                \includegraphics[height=4.5cm]{assets/connellSignalProtocolPostQuantum2025_state.png}
            \end{figure}
            }<2-4>
        \end{column}
    \end{columns}
    \only{
    \begin{figure}
        \includegraphics[width=\textwidth]{assets/size.pdf}
    \end{figure}
    }<4>
\end{frame}
